...
T["D3.2 . after a blockwise g! matches Vim's own redo of gU"] = function()
  ...
  -- line that starts with a <Tab>; the shape that shows it carries the reason
  -- as its fourth entry and is skipped there with a note, and runs everywhere
  -- else.
  local cases = {
    {
      { "abcd", "efgh", "", "abcd", "efgh" },
    ...
    -- back from two corners would take the whole tab and everything under it.
    {
      ...
      { "5G", "0", "<C-v>", "2j" },
      { "1G", "0" },
      "Neovim nightly's own redo differs on a tab-led line (#12)",
    },
  }
  for i, c in ipairs(cases) do
    ...
    if c[4] and child.fn.has("nvim-0.13") == 1 then
      MiniTest.add_note(("case %d skipped: %s"):format(i, c[4]))
    else
      select_region(entry)
      child.type_keys("gU")
